...
class ImportGaussianSplatting(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        log_opacity_attr = mesh.attributes.new(
            name="log_opacity", type="FLOAT", domain="POINT"
        )
        ...
        logscale_attr = mesh.attributes.new(
            name="logscale", type="FLOAT_VECTOR", domain="POINT"
        )
        ...
        for j in range(0, 15):
            sh_attr = mesh.attributes.new(
                name=f"sh{j + 1}", type="FLOAT_VECTOR", domain="POINT"
            )
            if features_extra.size > 0:
                sh_attr.data.foreach_set("vector", features_extra[:, :, j].flatten())

        rot_quatxyz_attr = mesh.attributes.new(
            name="quatxyz", type="FLOAT_VECTOR", domain="POINT"
        )
        ...
        rot_euler_attr = mesh.attributes.new(
        ...
        for j in range(0, 16):
            ...
            mat_tree.links.new(
                sh_inst_attr_node.outputs["Vector"], vector_math_node.inputs[0]
            )

            mat_tree.links.new(
                sh_geom_attr_node.outputs["Vector"], vector_math_node.inputs[1]
            )
        ...
        mat_tree.links.new(
            opacity_geom_attr_node.outputs["Fac"], opacity_attr_node.inputs[0]
        )

        mat_tree.links.new(
            opacity_inst_attr_node.outputs["Fac"], opacity_attr_node.inputs[1]
        )
        ...
        mat_tree.links.new(
            combine_xyz_node.outputs["Vector"], vector_transform_node.inputs["Vector"]
        )
        ...
        # mat_tree.links.new(y, yz_node.inputs[0])
        # mat_tree.links.new(z, yz_node.inputs[1])
        ...
        # mat_tree.links.new(x, xz_node.inputs[0])
        # mat_tree.links.new(z, xz_node.inputs[1])g
        ...
        mat_tree.links.new(
            geometry_node.outputs["Incoming"], vector_math_node.inputs[1]
        )
        ...
        geo_tree = bpy.data.node_groups.new(
            name="GaussianSplatting", type="GeometryNodeTree"
        )
        ...
        if "Probability" in random_value_node.inputs:
            random_value_node.inputs["Probability"].default_value = min(
                RECOMMENDED_MAX_GAUSSIANS / N, 1
            )
        else:
        ...
        geo_tree.links.new(
            group_input_node.outputs["Geometry"], mesh_to_points_node.inputs["Mesh"]
        )

        geo_tree.links.new(
            is_point_cloud_node.outputs["Boolean"], maximum_node.inputs[0]
        )

        geo_tree.links.new(random_value_node.outputs[3], maximum_node.inputs[1])

        geo_tree.links.new(
            opacity_attr_gn_node.outputs["Attribute"], threshold_node.inputs[0]
        )
        ...
        geo_tree.links.new(
            join_selection_node.outputs[0], mesh_to_points_node.inputs["Selection"]
        )

        geo_tree.links.new(
            ico_node.outputs["Mesh"], set_shade_smooth_node.inputs["Geometry"]
        )

        geo_tree.links.new(
            set_shade_smooth_node.outputs["Geometry"], instance_node.inputs["Instance"]
        )

        geo_tree.links.new(
            is_point_cloud_node.outputs["Boolean"], switch_node.inputs[0]
        )
        ...
        geo_tree.links.new(
            set_point_radius_node.outputs["Points"], switch_node.inputs[2]
        )

        geo_tree.links.new(switch_node.outputs[0], set_material_node.inputs["Geometry"])

        geo_tree.links.new(
            mesh_to_points_node.outputs["Points"], instance_node.inputs["Points"]
        )
        ...
        geo_tree.links.new(
            avg_node.outputs["Value"], set_point_radius_node.inputs["Radius"]
        )
        ...
        geo_tree.links.new(
            rot_euler_attr.outputs["Attribute"], instance_node.inputs["Rotation"]
        )
...
class GaussianSplattingPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator(
            ImportGaussianSplatting.bl_idname, text="Import Gaussian Splatting"
        )
        ...
        if obj is not None and "gaussian_splatting" in obj:
            ...
            row = layout.row()
            row.prop(
                obj.modifiers["Geometry Nodes"].node_group.nodes.get("Boolean"),
                "boolean",
            ...
            col2 = row.column()
            col1.prop(
                obj.modifiers["Geometry Nodes"]
                .node_group.nodes["Named Attribute"]
                .inputs[0],
                "default_value",
                text="Attribute",
            )
            ...
            if (
                not obj.modifiers["Geometry Nodes"]
                .node_group.nodes.get("Boolean")
                .boolean
            ):
                row = layout.row()
                row.prop(
                    obj.modifiers["Geometry Nodes"]
                    .node_group.nodes.get("Random Value")
                    .inputs["Probability"],
                    "default_value",
            ...
            row = layout.row()
            row.operator(
                ExportGaussianSplatting.bl_idname, text="Export Gaussian Splatting"
            )
...
class ApplyTransform(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.rotation:
            ...
            quat = np.concatenate(
                [quatw.reshape(-1, 1), quatxyz.reshape(-1, 3)], axis=1
            )
...
def draw_gs_apply_transforms(self, context):
    ...
    prop = self.layout.operator(
        "object.apply_transforms_gs", text="Location (Gaussian Splatting)"
    )
    prop.location = True
    prop.rotation = False
    prop.scale = False
    ...
    prop = self.layout.operator(
        "object.apply_transforms_gs", text="Rotation (Gaussian Splatting)"
    )
    prop.location = False
    prop.rotation = True
    prop.scale = False
    ...
    prop = self.layout.operator(
        "object.apply_transforms_gs", text="Scale (Gaussian Splatting)"
    )
    prop.location = False
    prop.rotation = False
    prop.scale = True
...
def register():
    ...
    bpy.types.Scene.ply_file_path = bpy.props.StringProperty(
        name="PLY Filepath", subtype="FILE_PATH"
    )
